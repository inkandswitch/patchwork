...
import { InstrumentScheduler, globalInstrumentSchedulers, Step } from "./instrument-scheduler";
...
export function toggleFn(stepGrid: Step[], setPlayingIdx: (idx: number) => void, setIsPlaying: (isPlaying: boolean) => void, setPlayStartTime: (time: number) => void, incrementToggleAges: () => void, config: SongConfig): (instrumentVolume: number, drumVolume: number) => void {
    let wrkr: Worker;
    return (instrumentVolume, drumVolume) => {
        ...
        let instrumentSamplePlayerConfig = new InstrumentSamplePlayerConfig(config.instrument);
        let drumSamplePlayerConfig = new DrumSamplePlayerConfig(config.drum);
        let instrumentScheduler = new InstrumentScheduler(setPlayingIdx, instrumentSamplePlayerConfig, drumSamplePlayerConfig, instrumentVolume, drumVolume, incrementToggleAges);
        instrumentScheduler.initContext(audioContext);
        if (config.overridingInstrument) {
            console.log(config.overridingInstrument);
            import(config.overridingInstrument).then(
                (mod) => {
        ...
        const startTime = audioContext.currentTime + 0.050;
        instrumentScheduler.prepare_new_schedule(stepGrid, startTime, config);

        if (window.Worker) {
            wrkr = new Worker(new URL('/workers/sequencer-worker.js', import.meta.url));
            wrkrs.push(wrkr);
