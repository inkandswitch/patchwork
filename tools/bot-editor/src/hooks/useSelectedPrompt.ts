...
export type AIEditPrompt = LoadedPlugin<AIEditPromptDescription, AIEditPromptImplementation>;
...
export const useSelectedPrompt = (dataType: DataType | undefined) => {
  ...
  const allPrompts = usePluginDescriptions<AIEditPromptDescription, AIEditPromptImplementation>(
    "patchwork:ai-prompt"
  );
  ...
  useEffect(() => {
    const loadPrompts = async () => {
      const registry = getRegistry<AIEditPromptDescription>("patchwork:ai-prompt");
  ...
  const prompts = useMemo(() => {
    ...
    return loadedPrompts.filter((prompt) => {
