...
export class OverlayRepo implements RepoLike {
  ...
  readonly #wrapped = new Map<DocumentId, OverlayHandle<unknown>>();
  readonly #inner = new Map<DocumentId, DocumentProgress<unknown>>();
  readonly #resolving = new Map<DocumentId, Promise<OverlayHandle<unknown>>>();
  ...
  get handles(): Record<DocumentId, DocHandle<unknown>> {
    ...
    for (const [documentId, wrapped] of this.#wrapped) {
  ...
  async find<T>(id: AnyDocumentId): Promise<DocHandle<T>> {
    ...
    const cached = this.#wrapped.get(documentId);
    if (cached) return cached as unknown as DocHandle<T>;
    const wrapped = await this.#resolve<T>(original);
  ...
  findWithProgress<T>(id: AnyDocumentId): DocumentProgress<T> {
    ...
    return {
      ...
      subscribe: (callback) => {
        ...
        wrappedPromise.then(() => {
          const inner = self.#inner.get(documentId);
      ...
      get progress() {
        return self.#inner.get(documentId)?.progress;
      },
      get error() {
        const s = peek();
  ...
  // De-dupes concurrent resolutions of the same document: the first caller
  // dispatches the subscription, everyone else awaits the same promise.
  #resolve<T>(original: AutomergeUrl): Promise<OverlayHandle<T>> {
    const documentId = parseAutomergeUrl(original).documentId;
    const existing = this.#resolving.get(documentId);
    ...
    const promise = (async () => {
      ...
      const backingUrl = descriptor.cloneUrl ?? descriptor.url;
      const inner = this.baseRepo.findWithProgress<T>(backingUrl);
      this.#inner.set(documentId, inner as DocumentProgress<unknown>);
      const backing = await this.baseRepo.find<T>(backingUrl);
      const wrapped = new OverlayHandle<T>({
        presentedUrl: descriptor.url,
        backing,
      });
      this.#wrapped.set(documentId, wrapped as OverlayHandle<unknown>);
      return wrapped;
    })();

    this.#resolving.set(documentId, promise as Promise<OverlayHandle<unknown>>);
    return promise;
  }
}
...
// identity, the unwrapping `merge`, and the re-stamping EventEmitter surface.
...
export class OverlayHandle<T> {
  ...
  // Unwrap an overlay argument so the backing handles merge directly (Automerge
  // rejects a foreign wrapper object).
  merge(other: DocHandle<T>): void {
    const inner =
      other instanceof OverlayHandle
        ? (other as OverlayHandle<T>).backingHandle
        : other;
    this.#handle.merge(inner as DocHandle<T>);
  }
