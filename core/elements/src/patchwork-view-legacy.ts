...
/**
 * The doc-url / tool-id-driven view element. Mounts a tool plugin for the
 * document at `doc-url`, choosing the tool by `tool-id` (or, when absent,
 * the document's `@patchwork.fallbackToolId`). Kept as a standalone
 * element so that `<patchwork-view>` can transparently delegate to it
 * when given legacy attributes — see `patchwork-view.ts`.
 */
  ...
  customElements.define(
    ...
    class PatchworkViewLegacyElement extends HTMLElement {
      ...
      connectedCallback() {
        ...
        // Default to layout-transparent so that when this element is used
        // as a child of `<patchwork-view>` (the wrapper's delegation
        // target), tool content participates in the wrapper's layout box
        // directly. Direct standalone usage can override via inline style
        // or a CSS rule on `patchwork-view-legacy`.
      ...
      #resetDisplay = () => {
        ...
        // Don't clear `display`: we intentionally set it to `contents` in
        // connectedCallback so layout passes through to the wrapper.
