...
const Tool: React.FC<ReactToolProps> = ({ docUrl }) => {
  ...
  const allActions = usePluginDescriptions<any>("patchwork:action");

  // Filter actions that match the current datatype
    ...
    const initDefaults = async () => {
      ...
      for (const action of actions) {
        try {
          ...
          console.error(`Failed to load plugin ${action.id}:`, err);
        }
      }
