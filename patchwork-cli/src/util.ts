...
export const getPatchworkConfig = () => {
  const currentDir = process.cwd();

  const configFilePath = path.join(currentDir, "patchwork.json");

  if (fs.existsSync(configFilePath)) {
    try {
      const configFileContents = fs.readFileSync(configFilePath, "utf8");
      return JSON.parse(configFileContents) as PatchworkProjectConfig;
    } catch (error) {
      console.warn("invalid patchwork.json file");
      return null;
    }
  } else {
    return null;
...
// Config file storage
const CONFIG_FILE_PATH = path.join(os.homedir(), ".patchwork", "config.json");
...
export function getConfig(): PatchworkConfig {
  try {
...
export function setConfig(config: Partial<PatchworkConfig>) {
