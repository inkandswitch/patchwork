  ...
  customElements.define(
    ...
    class PatchworkViewElement extends HTMLElement {
      ...
      #render() {
        ...
        if (!toolId) {
          // If a getFallbackTool callback is provided and we haven't
          ...
          // re-render.
          if (params.getFallbackTool && !this.#resolving && this.docUrl) {
            this.#resolving = true;
            this.#state = "unable";
            this.#displayLoading("...");
            params.getFallbackTool(this.docUrl).then((result) => {
              this.#resolving = false;
              ...
              this.#resolving = false;
              console.error("getFallbackTool failed:", err);
            });
            return;
          }
