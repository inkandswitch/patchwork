...
describe("Ref", () => {
  ...
  describe("isChildOf", () => {
    it("should return true for direct array element children", () => {
      handle.change((d) => {
        d.todos = [{ title: "Task", done: false }];
      });
      ...
      expect(todoRef.isChildOf(todosRef)).toBe(true);
    });

    it("should return false for grandchildren", () => {
      handle.change((d) => {
        d.todos = [{ title: "Task", done: false }];
      });
      ...
      expect(titleRef.isChildOf(todosRef)).toBe(false);
    });

    it("should return false for parent (reverse direction)", () => {
      handle.change((d) => {
        d.todos = [{ title: "Task" }];
      });
      ...
      expect(todosRef.isChildOf(todoRef)).toBe(false);
    });

    it("should return true for text range children", () => {
      handle.change((d) => {
        d.content = "Hello World";
      });

      const textRef = ref(handle, "content");
      const rangeRef = ref(handle, "content", [0, 5]);

      expect(rangeRef.isChildOf(textRef)).toBe(true);
    });

    it("should return false for text ranges with different paths", () => {
      handle.change((d) => {
        d.text1 = "Hello";
        d.text2 = "World";
      });

      const text1Ref = ref(handle, "text1");
      const text2Range = ref(handle, "text2", [0, 5]);

      expect(text2Range.isChildOf(text1Ref)).toBe(false);
    });

    it("should return false for same ref", () => {
      handle.change((d) => {
        d.value = 42;
      });

      const valueRef = ref(handle, "value");

      expect(valueRef.isChildOf(valueRef)).toBe(false);
    });
    ...
    it("should return false for different documents", () => {
      ...
      const itemsRef1 = ref(handle, "items");
      const itemRef2 = ref(handle2, "items", 0);

      expect(itemRef2.isChildOf(itemsRef1)).toBe(false);
    });
    ...
    it("should return false for different heads", () => {
      ...
      const itemsRef = ref(handle, "items").viewAt(heads1);
      const itemRef = ref(handle, "items", 0).viewAt(heads2);

      expect(itemRef.isChildOf(itemsRef)).toBe(false);
    });
    ...
    it("should work with stable refs (ObjectIds)", () => {
      ...
      expect(itemRef.isChildOf(itemsRef)).toBe(true);
    });
    ...
    it("should work with id patterns", () => {
      ...
      const usersRef = ref(handle, "users");
      const userRef = ref(handle, "users", { id: "alice" });

      expect(userRef.isChildOf(usersRef)).toBe(true);
    });
  });
