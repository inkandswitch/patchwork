...
export const useSyncDocTitle = ({ selectedDocPath, repo, selectDocPath, }) => {
    ...
    const selectedDocTitleRef = useRef();
    const dataType = dataTypeById(selectedDocLink?.type);
    const account = useCurrentAccount();
    const selectedDoc = useAsyncComputed(useCallback(() => {
        fetchAwaitMissing(selectedDocPath);
        return fetchOmOnActiveBranch(selectedDocPath, account, repo).doc;
    }, [account, repo, selectedDocPath])).ifPending(undefined).value;
    const parentFolderOm = useAsyncComputed(useCallback(() => {
        fetchAwaitMissing(selectedDocPath);
        return fetchOmOnActiveBranch(DocPathUtils.parent(selectedDocPath), account, repo);
    }, [account, repo, selectedDocPath])).ifPending(undefined).value;
    useEffect(() => {
        ...
        dataType.getTitle(selectedDoc, repo).then((title) => {
            ...
            if (selectedDocLink.url !== selectedDocTitleRef.current?.url ||
            ...
            if (title !== selectedDocTitleRef.current?.title) {
                ...
                parentFolderOm.handle.change((d) => {
                    const existingDocLink = d.docs.find((link) => link.url === selectedDocLink.url);
