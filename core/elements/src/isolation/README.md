...
Additionally, host-origin `<link>` elements added to the DOM are intercepted by a MutationObserver — since native `<link>` elements make direct browser requests that bypass the `fetch()` override (and CORS-fail from the opaque origin), they are handled specially:

- **`rel="stylesheet"`** — replaced with a `<style>` tag whose content is fetched through the proxy.
- **`rel="modulepreload"`** — converted to `rel="modulepreload-shim"` **synchronously at insertion time** (by patching `Node.prototype.appendChild`/`insertBefore`), not via the MutationObserver. Code-split bundles (e.g. Vite's `__vitePreload`) inject a plain `modulepreload` link per chunk; the browser begins the native preload fetch the instant the element is inserted, which CORS-fails from the opaque origin — and that happens before any async observer callback can run, so the node must be mutated before it enters the DOM. The browser ignores the unknown `modulepreload-shim` rel (no native fetch), while es-module-shims honors it and preloads the chunk through its source hook (and thus the `fetch-package` RPC). The chunk also loads via the dynamic `import()` (which es-module-shims rewrites to `importShim` in shim mode); the shim preload just warms its cache in parallel.
