...
import { Node, FlatDocPathsContext } from "./Node";
const prepareDataForTree = (folderDoc, folderPath) => {
    if (!folderDoc) {
        return [];
    }
    return uniqBy(folderDoc.docs, "url").map((docLink) => {
        ...
        return {
            ...
            children: docLink.type === "folder" && docLink.folderContents
                ? prepareDataForTree(docLink.folderContents, childDocPath)
                : [],
        };
    });
};
const idAccessor = (data) => {
    return DocPathUtils.toString(data.docPath);
};
export const Sidebar = ({ selectedDocPath, selectDocPath, hideSidebar, addNewDocument, rootFolderDoc, }) => {
    const repo = useRepo();
    const dataTypes = useDataTypes();
    const { doc: rootFolderDocWithChildren, rootFolderUrl, flatDocPaths, } = rootFolderDoc;
    const [searchQuery, setSearchQuery] = useState("");
    const [accountDoc] = useCurrentAccountDoc();
    const [uiStateDoc, changeUIStateDoc] = useDocument(accountDoc?.uiStateUrl);
    const account = useCurrentAccount();
    const onMove = async ({ parentNode, index: dragTargetIndex, dragNodes, }) => {
        ...
        for (const dragNode of dragNodes) {
            ...
            const srcParentOm = await asyncComputedPromise(() => fetchOmOnActiveBranch(srcParentPath, account, repo));
            const dragItemIndex = srcParentOm.doc.docs.findIndex((item) => item.url === DocPathUtils.toLink(dragNode.data.docPath).url);
            if (dragItemIndex === -1) {
                throw new Error("Couldn't find drag item in parent folder");
            }
            const dstParentPath = !parentNode || parentNode.level < 0
                ? DocPathUtils.forRoot(rootFolderUrl)
                : parentNode.data.docPath;
            const dstParentOm = await asyncComputedPromise(() => fetchOmOnActiveBranch(dstParentPath, account, repo));
            // Time for the subtlety listed under #3 above...
            const overrideUrl = await asyncComputedPromise(() => {
                const srcBranchInfo = fetchBranchScopeAndActiveBranchInfo(srcPath, account, repo);
                if (srcBranchInfo.activeBranchOm && // we're on a branch
                    srcBranchInfo.branchScopeOm.url !== srcUrl // the branch scope lies above
                ) {
                    const dstBranchInfo = fetchBranchScopeAndActiveBranchInfo(dstParentPath, account, repo);
                    if (dstBranchInfo.activeBranchOm?.url !==
            ...
            const adjustedTargetIndex = DocPathUtils.toString(srcParentPath) ===
    ...
    const dataForTree = prepareDataForTree(rootFolderDocWithChildren, DocPathUtils.forRoot(rootFolderUrl));
    ...
    const onRename = async ({ node, name }) => {
        ...
        if (!dataType?.setTitle) {
            alert(`${capitalize(dataType.name)} documents can only be renamed in the main editor, not the sidebar.`);
            return;
        }
        if (!docLink) {
            return;
        }
        const docOm = await asyncComputedPromise(() => fetchOmOnActiveBranch(docPath, account, repo));
        const parentPath = DocPathUtils.parent(docPath);
        const parentOm = await asyncComputedPromise(() => fetchOmOnActiveBranch(parentPath, account, repo));
        // rename doc link
    ...
    const initialOpenState = useMemo(() => (uiStateDoc?.docPathsToggledOpenInSidebar ?? []).reduce((acc, key) => {
        acc[key] = true;
        return acc;
    }, {}), [uiStateDoc]);
    // Show a loading spinner until we've recursively loaded all folder contents
    if (rootFolderDocWithChildren === undefined) {
        return (_jsx("div", { className: "flex items-center justify-center h-screen", children: _jsx("p", { className: "text-gray-400 text-sm", children: "Loading..." }) }));
    }
    return (_jsxs("div", { className: "flex flex-col h-screen", children: [_jsxs("div", { className: "h-10 py-2 px-4 font-semibold text-gray-500 text-sm flex", children: [_jsx("div", { className: "mw-40 mt-[3px]", children: "My Documents" }), _jsx("div", { className: "ml-auto", children: _jsx("div", { className: "text-gray-400 hover:bg-gray-300 hover:text-gray-500 cursor-pointer  transition-all p-1 m-[-4px] mr-[-8px] rounded-sm", onClick: hideSidebar, children: _jsx(ChevronsLeft, {}) }) })] }), _jsxs("div", { className: "py-2  border-b border-gray-200", children: [_jsx(DataTypeSelector, { dataTypes: dataTypes, addNewDocument: addNewDocument }), _jsx(OpenAutomergeUrl, { addNewDocument: addNewDocument }), _jsxs("div", { className: `py-1 px-2 text-sm  cursor-pointer ${selectedDocPath &&
                            DocPathUtils.toLink(selectedDocPath).type === "my-tools"
                            ? " bg-gray-300 hover:bg-gray-300 text-gray-900"
                            : "text-gray-600 hover:bg-gray-200"}`, onClick: () => openModuleSettings(), children: [_jsx(Icon, { type: "Cog", size: 14, className: "inline-block font-bold mr-2 align-top mt-[2px]" }), "My Tools"] })] }), _jsxs("div", { className: "mx-2 my-2 flex gap-2 items-center", children: [_jsx(Input, { placeholder: "Search my docs...", className: "h-6", value: searchQuery, onChange: (e) => setSearchQuery(e.target.value) }), _jsx("div", { className: `text-gray-400 text-xs cursor-pointer ${searchQuery.length > 0 ? "" : "invisible"}`, onClick: () => setSearchQuery(""), children: "Clear" })] }), _jsx("div", { className: "flex-grow overflow-auto", children: _jsx(FillFlexParent, { children: ({ width, height }) => {
                        return (_jsx(FlatDocPathsContext.Provider, { value: flatDocPaths, children: _jsx(Tree, { data: dataForTree, width: width, height: height, openByDefault: false, searchTerm: searchQuery, searchMatch: (node, term) => {
                                    const docName = DocPathUtils.toLink(node.data.docPath).name;
                                    return docName.toLowerCase().includes(term.toLowerCase());
                                }, rowHeight: 28, selection: treeSelection, idAccessor: idAccessor, onSelect: (selections) => {
                                    if (!selections ||
                                        selections.length === 0 ||
                                ...
                                initialOpenState: initialOpenState, onToggle: onToggle, onRename: onRename, children: Node }) }));
                    } }) }), _jsx("div", { className: "h-12 border-t border-gray-300 py-1 px-2 bg-gray-200", children: _jsx(AccountPicker, { showName: true }) })] }));
};
