...
- [ ] **Isolate multiple sister components together.** Adjust the component architecture so that `patchwork-isolation` can wrap multiple components that reference multiple documents. This lets providers communicate with each other within the isolation boundary, enabling history/comments/branches without tackling the harder (deferred) problem of providers accross the boundary. In particular, instead of having a frame tool where `patchwork-isolation` wraps the main document view, it should wrap both the main document view and the context sidebar. The main change needed is in how the allowlist gets populated.
